...
export function CharacterSheetPage() {
  ...
  const subclassEntry = findSubclass(cls, character?.subclass ?? '');
  const species = useSpeciesEntry(character?.species ?? '');
  const background = useBackgroundEntry(character?.background ?? '');
  ...
  const update: UpdateCharacter = (patch) => saveCharacter({ ...character, ...patch });
