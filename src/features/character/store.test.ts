...
describe('character store', () => {
  ...
  it('merges rapid patches into the latest character state', () => {
    const id = useCharacterStore.getState().addCharacter('Hero');

    useCharacterStore.getState().updateCharacter(id, { name: 'Updated' });
    useCharacterStore.getState().updateCharacter(id, { level: 4 });

    expect(useCharacterStore.getState().characters[id]).toEqual(
      expect.objectContaining({ name: 'Updated', level: 4 }),
    );
  });
